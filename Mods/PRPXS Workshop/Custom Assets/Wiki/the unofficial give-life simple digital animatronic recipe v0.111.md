...
lastly, its recommended to use github desktop to clone the game repository so you can just hit "pull origin" whenever 64 drops updates


---
navbox:
- [[#0. hotkeys guide (scroll down)]]
- [[#1. make the body (the mesh)]]
- [[#2. make the skeleton (the armature)]]
- [[#3. parent it (link the armature to mesh)]]
- [[#4. weight-painting]]
- [[#5. test da bones]]
- [[#6. animations]]
- [[#8. exporting]]
...
- [[#11. grouping]]
- [[#12. halfway!]]
- [[#13. proper animation support]]
- [[#14. writing the animations]]
- [[#15. other stuff]]
---
...
shftA - create a bone or mesh out of thin air
### armature:
ctrlP - parents an object; puts under a hood/connects as relation

### mesh:
ctrlJ - merges meshes together; mesh glue
...
0. its good to have a collection of references or concept art to prepare your idea
	optional, and may be a bit hectic for starting out or simpler ideas, but helps on certain cases
	for more information: https://undertheweathersoftware.com/turning-concept-art-into-a-3d-model-in-blender/


	for beginners, modelling and otherwise, it is reccomended to start with something small to avoid burnout. it is good to have other plans to build up stairs to big projects.

###### the next section will roughly go into modelling in Blender, designed to fit for beginners/people who haven't modeled in a long time, but it is reccomended to use a different guide for more clearer instructions. see something like this tutorial playlist: https://www.youtube.com/playlist?list=PLsGl9GczcgBumq6fLmmR1ZYYn4EDkIg5B

---
## **1. make the body (the mesh)**

###### if you already have a model made, you can skip to part 7.

- add mesh pieces (cube sphere etc) and shape them into parts; head torso arms etc
    - keep them as _separate objects_ at first! merging after rigging is good for optimization, but keeping them separate makes it easier to do most of the modeling we'll be doing.
    if you want to add support for small alternative skins that dont need a whole ass model (like clothes-swapping or small character edits- your pobably gonna have to export it as one anyway), having a backup of a ver pre-merging is a good idea

- add a subdivide modifier (blue wrench tab), this makes cubes turn smooth. helps a lot with modeling
	- apply it after your done making the mesh part, then in edit mode unsubdivide it for optimizing

- after a bit (maybe after finishing the rig), make sure to go back to edit mode and press `opt(/alt) N` to recalculate normals so the export doesnt look weird.

## **2. make the skeleton (the armature)

###### if you already have a rig, skip to part 7.

- shift+A → armature → single bone, then go into edit mode
    - A: extrude bones (E) to build: spine → neck → head, shoulders → arms → hands, hips → legs → feet.
	    - you could bone only the left side, then shift-A (?) -> symmetrize to easily duplicate it to the right.
	- B: use rigify (blender plugin, find it in settings → addons), add a humanoid rig & contort it to your character.

don’t worry about perfection for this part.

- move bones so they sit *inside* the mesh
	this helps with weight-painting later, especially if using automatic methods
	if you cant see the bones after that, in the bone view make sure their 'visible infront'
    
- switch between object/edit mode if needed
    you could also reference the bone's position using the viewport angles (in the top right corner)
    ![[Screenshot 2026-05-03 at 6.25.36 PM.png]]


- select your mesh pieces, hold `shift`, select the armature last, and hit `ctrl + p`!
	→ **with automatic weights**: convenient (but might require fine-tuning)
	→ **with no weights**: best for simple/small designs (weight-painting diy)

### 4. weight-painting

- essentially acts like... acupuncture painting.
- with auto-weights you might have to finetune it's results.

- you paint how much you want a mesh (& its details) to move.
	- red means all of it will move, green means it will softly move & follow along. blue means it wont at all, so no
-# it looks similar to thermal radars seen on weather channels.
...
- grab & rotate (R) a few bones
	- tip: double press R for mouse-based rotation
    

it will look WRONG
that’s normal
like limbs stretching into spaghetti
